...
module.exports = {
  ...
  create(context) {
    ...
    return {
      [`JSXElement[openingElement.name.name=/((^b|B)utton|Anchor|Link|^a)/]:has(JSXClosingElement):not(:has(:matches(JSXAttribute[name.name=/^(text|alt|aria-label(ledby)?)$/]:not(:matches([value=null],[value.value=""])),JSXText,JSXExpressionContainer,JSXOpeningElement[name.name=/(SmartHRLogo|Text|Message${customComponents})$/])))`]:
        (node) => {
          context.report({
            node,
