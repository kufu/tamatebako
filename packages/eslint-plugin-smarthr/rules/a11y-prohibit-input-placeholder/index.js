...
module.exports = {
 ...
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-prohibit-input-placeholder`,
      ...
      [`${COMBOBOX_ELEMENT}:not(:has(${DEFAULT_ITEM_ATTRIBUTE}, JSXAttribute[name.name="dropdownHelpMessage"])) ${PLACEHOLDER_ATTRIBUTE}`]:
        (node) => {
 ...
 - 上記以外の説明を行いたい場合、ヒント用要素を設置してください。(例: '<div><${name} /><Hint>ヒント</Hint></div>')`,
      ...
      [`JSXOpeningElement${SEARCH_INPUT_NAME}:not(:has(JSXAttribute[name.name="tooltipMessage"])) ${PLACEHOLDER_ATTRIBUTE}`]: (
        node,
      ) => {
 ...
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-prohibit-input-placeholder`,
      ...
      [`JSXOpeningElement[name.name=/((I|^i)nput|(T|^t)extarea|(Date|Wareki|Time)Picker)$/]:not(${SEARCH_INPUT_NAME}) ${PLACEHOLDER_ATTRIBUTE}`]:
        (node) => {
          const name = node.parent.name.name
