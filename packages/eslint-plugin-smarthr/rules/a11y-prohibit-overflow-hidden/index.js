...
module.exports = {
  ...
  create(context) {
    return {
      'JSXAttribute[name.name="overflow"]:matches([value.value="hidden"],[value.expression.value="hidden"],[value.expression.quasis.length=1][value.expression.quasis.0.value.cooked="hidden"])':
        (node) => {
          context.report({
            node,
