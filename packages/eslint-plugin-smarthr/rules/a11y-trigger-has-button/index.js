...
const filterFalsyJSXText = (cs) => cs.filter((c) => !(c.type === 'JSXText' && FALSY_TEXT_REGEX.test(c.value)))
...
module.exports = {
  ...
  create(context) {
    return {
      [`JSXElement[openingElement.name.name=${TRIGGER_REGEX}]:not([openingElement.name.name=/HelpDialogTrigger$/])`]: (
        parentNode,
      ) => {
