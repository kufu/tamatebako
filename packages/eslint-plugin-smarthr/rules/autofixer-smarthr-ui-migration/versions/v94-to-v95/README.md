...
- 以下のパターンは自動修正できません。エラーメッセージを確認して、手動で修正してください：
  ...
  - 引数ありの関数: `decorators={{ closeButtonLabel: (lang) => ... }}`
  - BlockStatement形式の関数: `decorators={{ closeButtonLabel: () => { return "OK" } }}`
  - AppLauncherの`decorators.triggerLabel`から`triggerLabel`への値の移行（既に`triggerLabel`属性がある場合は自動削除されます）
