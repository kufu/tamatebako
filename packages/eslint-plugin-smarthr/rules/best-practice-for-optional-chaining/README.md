...
関数の存在確認をif文で行ってから実行する、というパターンは以下のように簡潔に書けます:
...
optional chainingを使うことで、以下のメリットがあります:

- **コードが簡潔になる**
  - if文とその中のブロックが不要になり、1行で表現できる

- **意図が明確になる**
  - 「関数が存在すれば実行する」という意図が一目で分かる
  - if文だと他の処理が含まれる可能性を考慮する必要がある

- **タイプミスのリスクが減る**
  - if文の条件とブロック内で同じ変数を2回書く必要がない
