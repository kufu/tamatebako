...
Trigger - Dialogの関係は、その特性上、ひとつのDialogに対して複数のTriggerがある1:nか、DialogとTriggerが1:1の関係になる場合しか存在しません。そのため、Dialogの名称はそのプロダクトの中で常に一意になる文字列を指定できます。この明確な関係性を維持するためには、リテラル文字列で統一的に管理することが適切です。
