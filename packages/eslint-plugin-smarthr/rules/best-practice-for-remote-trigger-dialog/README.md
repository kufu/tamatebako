...
Trigger - Dialogの関係は、その特性上、ひとつのDialogに対して複数のTriggerがある1:nか、DialogとTriggerが1:1の関係になる場合しか存在しません。1:nになる理由は、ループ内などで都度Dialogを定義することはパフォーマンス上の観点から推奨されておらず、基本的に同一のDialogの内容を書き換えることが推奨されているためです。そのため、Dialogの名称はそのプロダクトの中で常に一意になる文字列を指定できます。この明確な関係性を維持するためには、リテラル文字列で統一的に管理することが適切です。

以上から、変数を使わない事によってDialogとTriggerが指すIDは常に一意の文字列になり、コード検索などで簡単に紐づく対象を見つけることができます。
