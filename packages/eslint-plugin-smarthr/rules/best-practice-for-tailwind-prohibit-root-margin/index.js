...
module.exports = {
  ...
  create(context) {
    return {
      ':matches(ArrowFunctionExpression,FunctionDeclaration,ReturnStatement) > JSXElement > JSXOpeningElement JSXAttribute[name.name="className"][value.value=/( |^)shr-m[trbl]?-/]':
        (node) => {
          context.report({
            node,
