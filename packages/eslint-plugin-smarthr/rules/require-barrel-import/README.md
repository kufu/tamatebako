...
**異なるディレクトリ間（複数のbarrelが見つかった場合）:**
- **同じファイル名**（例: `client.ts`同士）: より親のbarrelを優先
- **異なるファイル名**（例: `index.ts` vs `client.ts`）: より近い（子に近い）barrelを優先

**階層の一貫性チェック:**
- 子ディレクトリで`client.ts`を使用している場合、親ディレクトリにも`client.ts`を作成することを促します
- これにより、プロジェクト全体でbarrel構造の一貫性を保ちます
