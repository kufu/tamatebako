...
const detectDirectBarrelImport = (importedPath, barrelFileNames) => {
  ...
  if (fs.existsSync(importedPath) && fs.statSync(importedPath).isDirectory()) {
    for (const ext of TARGET_EXTS) {
      const candidate = `${importedPath}/index.${ext}`
...
const checkAllowedImports = (node, importerDir, targetAllowedImports, allowedImportsConfig) => {
  ...
  // 完全に許可されている場合はスキップ
  const shouldSkip =
    (isDenyPath && deniedModules[0] === true) || (!isDenyPath && deniedModules.length === 1 && deniedModules[0].length === 0)
...
const findBarrelFile = (importedPath, importerDir, additionalBarrelFileNames = []) => {
  ...
  while (pathSegments.length > 0) {
    ...
    const searchFileNames = isAdditionalBarrelFound ? [foundBarrelFileName] : barrelFileNames

    // 現在のパスにbarrelファイルがあるかチェック
    let foundBarrel = null
    for (const candidate of generateBarrelFilePaths(currentPath, searchFileNames)) {
      if (fs.existsSync(candidate)) {
        foundBarrel = candidate
        break
      }
    }
    ...
    if (foundBarrel) {
      ...
      let indexBarrel = null
      for (const candidate of generateBarrelFilePaths(currentPath, ['index'])) {
        if (fs.existsSync(candidate)) {
          indexBarrel = candidate
          break
        }
      }
...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
      ...
      const allBarrelsInSameDir = generateBarrelFilePaths(barrelDir, barrelFileNames).filter(fs.existsSync)

      // directBarrelPathがある場合はそれを使用、ない場合は拡張子を追加して探す
      let isImportingFromBarrel = false
      if (directBarrelPath) {
        isImportingFromBarrel = allBarrelsInSameDir.includes(directBarrelPath)
      } else {
        for (const ext of TARGET_EXTS) {
          if (allBarrelsInSameDir.includes(`${importedPath}.${ext}`)) {
            isImportingFromBarrel = true
            break
          }
        }
      }
      ...
      if (isImportingFromBarrel) {
        ...
        let isImportingSelf = false
        if (directBarrelPath) {
          isImportingSelf = directBarrelPath === importerPath
        } else {
          for (const ext of TARGET_EXTS) {
            if (`${importedPath}.${ext}` === importerPath) {
              isImportingSelf = true
              break
            }
          }
        }
        ...
        let importedBarrelName = null
        if (directBarrelPath) {
          importedBarrelName = path.basename(directBarrelPath, path.extname(directBarrelPath))
        } else {
          for (const ext of TARGET_EXTS) {
            const name = path.basename(`${importedPath}.${ext}`, path.extname(`${importedPath}.${ext}`))
            if (barrelFileNames.includes(name)) {
              importedBarrelName = name
              break
            }
          }
        }
...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
      ...
      const barrelDirWithAlias = barrelWithAlias.replace(
        REGEX_INDEX_FILE.test(barrelWithAlias) ? REGEX_INDEX_FILE : REGEX_BARREL_FILE_EXT,
        '',
      )
      ...
      const barrelFilesToShow = hasAdditionalBarrels
        ? barrelFileNames.map((name) => {
            ...
            return candidates.find(fs.existsSync) || `${barrelDir}/${name}.ts`
          })
        : allBarrelsInSameDir
      ...
      const barrelSuggestions = barrelFilesToShow.map((filePath) => {
        ...
        const dirWithAlias = pathWithAlias.replace(
          REGEX_INDEX_FILE.test(pathWithAlias) ? REGEX_INDEX_FILE : REGEX_BARREL_FILE_EXT,
          '',
        )
        ...
        if (node.source.value[0] === '.') {
          const relativePath = path.relative(importerDir, resolvePathAlias(dirWithAlias))
      ...
      if (node.source.value[0] === '.') {
        ...
        const relativePath = path.relative(importerDir, resolvePathAlias(barrelDirWithAlias))
      ...
      if (shouldShowAllSuggestions) {
        const indexStartWithText = 'index.'
        // index.ts を優先的に表示するためにソート
        const sortedSuggestions = [...barrelSuggestions].sort((a, b) => {
          const isAIndex = a.fileName.startsWith(indexStartWithText)
          const isBIndex = b.fileName.startsWith(indexStartWithText)
        ...
        suggestionsMessage =
          ...
          sortedSuggestions
            .map(
              ({ importPath, fileName, exists }) =>
                `  - import { ${importedModules} } from '${importPath}' // ${fileName}${exists ? '' : ' (作成が必要)'}`,
            )
            .join('\n')

        // 存在しないファイルがある場合は注意メッセージを追加
        if (existingBarrels.length < barrelSuggestions.length) {
          suggestionsMessage += '\n\n※ 存在しないバレルファイルは必要に応じて作成してください。'
        }
      } else {
        suggestionsMessage = `\n推奨されるimport:  import { ${importedModules} } from '${suggestedImportPath}'`
      }
      const barrelFilesInfo =
        existingBarrels.length > 1
          ? existingBarrels.reduce((acc, { barrelFile }) => (acc ? `${acc}, ${barrelFile}` : barrelFile), '')
          : barrelWithAlias
...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
    ...
    // 重複exportチェックが必要かどうか
    // barrelFileNamesが2つ以上ある場合のみ、同じディレクトリに複数のバレルファイルが存在する可能性がある
    const shouldCheckDuplicateExports = barrelFileNames.length > 1

    // 現在のファイルがバレルファイルかチェック
    const currentFilePath = context.filename
    const currentIsBarrel = shouldCheckDuplicateExports && isBarrelFile(currentFilePath, barrelFileNames)

    // バレルファイルのexportを収集（重複チェック用）
    const currentBarrelExports = []
    ...
    return {
      ...
      ExportNamedDeclaration: (node) => {
        ...
        // バレルファイルからのre-exportを記録（重複チェック用）
        if (currentIsBarrel && node.source && node.specifiers && node.specifiers.length > 0) {
          // sourceファイルを絶対パスに変換
          const sourceRelative = node.source.value
          let sourceFile = path.resolve(importerDir, sourceRelative)
      ...
      'Program:exit': () => {
        // ファイルの処理が終わった後に重複チェックを実行
        // currentIsBarrelの定義でbarrelFileNames.length > 1も判定済み
        if (currentIsBarrel) {
          for (const { node, exportedName, siblingPath, sourceFile, importedName } of findDuplicateExports(
            currentFilePath,
            currentBarrelExports,
            barrelFileNames,
          )) {
            context.report({
              node,
              message: createDuplicateExportMessage(exportedName, currentFilePath, siblingPath, sourceFile, importedName),
            })
          }
        }
      },
