...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
      ...
      const barrelDirWithAlias = barrelWithAlias.replace(REGEX_INDEX_FILE.test(barrelWithAlias) ? REGEX_INDEX_FILE : REGEX_BARREL_FILE_EXT, '')
      ...
      const barrelSuggestions = barrelFilesToShow.map(filePath => {
        ...
        const dirWithAlias = pathWithAlias.replace(REGEX_INDEX_FILE.test(pathWithAlias) ? REGEX_INDEX_FILE : REGEX_BARREL_FILE_EXT, '')
...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
    ...
    return {
      ...
      'Program:exit': () => {
        ...
        if (currentIsBarrel) {
          for (const { node, exportedName, siblingPath, sourceFile, importedName } of findDuplicateExports(currentFilePath, currentBarrelExports, barrelFileNames)) {
            context.report({
              node,
