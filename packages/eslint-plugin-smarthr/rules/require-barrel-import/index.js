...
const detectDirectBarrelImport = (importedPath, barrelFileNames) => {
  ...
  if (fs.existsSync(importedPath) && fs.statSync(importedPath).isDirectory()) {
    const indexFile = TARGET_EXTS.reduce((found, ext) => {
  ...
  const fileWithExt = TARGET_EXTS.reduce((found, ext) => {
    if (found) return found
...
const findBarrelFile = (importedPath, importerDir, additionalBarrelFileNames = []) => {
  ...
  while (pathSegments.length > 0) {
    ...
    const foundBarrel = generateBarrelFilePaths(currentPath, searchFileNames)
      .reduce((found, candidate) => found || (fs.existsSync(candidate) ? candidate : null), null)
...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
      ...
      const isImportingFromBarrel = directBarrelPath
        ? allBarrelsInSameDir.includes(directBarrelPath)
        : TARGET_EXTS.reduce((found, ext) => found || allBarrelsInSameDir.includes(`${importedPath}.${ext}`), false)
      ...
      if (isImportingFromBarrel) {
        ...
        const isImportingSelf = directBarrelPath
          ? directBarrelPath === importerPath
          : TARGET_EXTS.reduce((found, ext) => found || `${importedPath}.${ext}` === importerPath, false)
        ...
        const importedBarrelName = directBarrelPath
          ? path.basename(directBarrelPath, path.extname(directBarrelPath))
          : TARGET_EXTS.reduce((found, ext) => {
              if (found) return found
              const name = path.basename(`${importedPath}.${ext}`, path.extname(`${importedPath}.${ext}`))
              return barrelFileNames.includes(name) ? name : null
            }, null)
