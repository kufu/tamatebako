...
詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/require-barrel-import`,
    ...
    /**
     * 同じディレクトリの他のバレルファイルとのexport重複をチェック
     */
    const checkDuplicateExports = () => {
      ...
      const duplicates = findDuplicateExports(currentFilePath, currentBarrelExports, barrelFileNames)

      for (const { node, exportedName, siblingPath } of duplicates) {
        context.report({
          node,
          message: createDuplicateExportMessage(exportedName, currentFilePath, siblingPath),
        })
      }
    }
    ...
    return {
      ...
      ExportNamedDeclaration: (node) => {
        ...
        if (currentIsBarrel && node.source) {
          // export { A, B } from './file' の形式
          if (node.specifiers && node.specifiers.length > 0) {
            for (const specifier of node.specifiers) {
              if (specifier.type === 'ExportSpecifier') {
                const exportedName = specifier.exported.name
                currentBarrelExports.push({ node: specifier, exportedName })
              }
            }
          }
          // export * from './file' の場合は重複チェック対象外（すべてをre-exportするため）
        }
      },
      'Program:exit': () => {
        // ファイルの処理が終わった後に重複チェックを実行
        // barrelFileNamesが2つ以上ある場合のみ実行
        if (shouldCheckDuplicateExports) {
