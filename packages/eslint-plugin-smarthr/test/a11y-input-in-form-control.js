...
const noLabeledInput = (
  name,
) => `${name} を、smarthr-ui/FormControl もしくはそれを拡張したコンポーネントが囲むようマークアップを変更してください。
...
const noLabeledSelect = (
 ...
 - ${name}が入力要素とラベル・タイトル・説明など含む概念を表示するコンポーネントの場合、コンポーネント名を/(Form(Control|Group)|(F|^f)ieldset)$/とマッチするように修正してください
 - ${name}が入力要素自体を表現するコンポーネントの一部である場合、ルートとなるコンポーネントの名称を/(RadioButton(Panel)?(s)?|Check(B|b)ox(es|s)?|(Search)?(I|^i)nput(File)?|(T|^t)extarea|(S|^s)elect|Combo(B|b)ox|(Date|Wareki|Time)Picker)$/とマッチするように修正してください`
const invalidPureCheckboxInFormControl = (
  name,
) => `HogeFormControl が ${name} を含んでいます。smarthr-ui/FormControl を smarthr-ui/Fieldset に変更し、正しくグルーピングされるように修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
 - 可能なら${name}はsmarthr-ui/Checkboxへの変更を検討してください。難しい場合は ${name} と結びつくlabel要素が必ず存在するよう、マークアップする必要があることに注意してください。`
const invalidCheckboxInFormControl = (
  name,
) => `HogeFormControl が ${name} を含んでいます。smarthr-ui/FormControl を smarthr-ui/Fieldset に変更し、正しくグルーピングされるように修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control`
const invalidPureRadioInFormControl = (
  name,
) => `HogeFormControl が ${name} を含んでいます。smarthr-ui/FormControl を smarthr-ui/Fieldset に変更し、正しくグルーピングされるように修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const invalidRadioInFormControl = (
  name,
) => `HogeFormControl が ${name} を含んでいます。smarthr-ui/FormControl を smarthr-ui/Fieldset に変更し、正しくグルーピングされるように修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
 - Fieldsetで同じname属性のラジオボタン全てを囲むことで正しくグループ化され、適切なタイトル・説明を追加出来ます`
const invalidMultiInputsInFormControl =
  () => `HogeFormControl が複数の入力要素を含んでいます。ラベルと入力要素の紐づけが正しく行われない可能性があるため、以下の方法のいずれかで修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const noLabeledInputInFieldset = (
  name,
) => `HogeFieldset が ラベルを持たない入力要素(${name})を含んでいます。入力要素が何であるかを正しく伝えるため、以下の方法のいずれかで修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const noLabeledInputInFieldsetWithSelect = (
  name,
) => `HogeFieldset が ラベルを持たない入力要素(${name})を含んでいます。入力要素が何であるかを正しく伝えるため、以下の方法のいずれかで修正してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
 - 方法1: HogeFieldset を smarthr-ui/FormControl、もしくはそれを拡張したコンポーネントに変更してください
 ...
 - 方法4: HogeFieldset が smarthr-ui/Fieldset、もしくはそれを拡張しているコンポーネントではない場合、名称を /Fieldset$/ にマッチしないものに変更してください
 - 方法5: 別途label要素が存在し、それらと紐づけたい場合はlabel要素のhtmlFor属性、${name}のid属性に同じ文字列を指定してください。この文字列はhtml内で一意である必要があります`
const useFormControlInsteadOfSection = (
  name,
  section,
) => `${name}は${section}より先に、smarthr-ui/FormControlが入力要素を囲むようマークアップを以下のいずれかの方法で変更してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const useFormControlInsteadOfSectionInRadio = (
  name,
  section,
) => `${name}は${section}より先に、smarthr-ui/Fieldsetが入力要素を囲むようマークアップを以下のいずれかの方法で変更してください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const invalidFieldsetHasRoleGroup = (
  fieldset,
  base,
) => `${fieldset}に 'role="group" が設定されています。${base} をつかってマークアップする場合、'role="group"' は不要です
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
 - ${fieldset} が ${base}、もしくはそれを拡張しているコンポーネントではない場合、名称を /(Form(Control|Group)|(F|^f)ieldset)$/ にマッチしないものに変更してください`
const invalidChildreninFormControl = (
  children,
) => `FormControl が、${children} を子要素として持っており、マークアップとして正しくない状態になっています。以下のいずれかの方法で修正を試みてください。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
const requireMultiInputInFormControlWithRoleGroup =
  () => `HogeFormControl内に入力要素が2個以上存在しないため、'role=\"group\"'を削除してください。'role=\"group\"'は複数の入力要素を一つのグループとして扱うための属性です。
 - 詳細: https://github.com/kufu/tamatebako/tree/master/packages/eslint-plugin-smarthr/rules/a11y-input-in-form-control
...
ruleTester.run('a11y-input-in-form-control', rule, {
  ...
  invalid: [
    ...
    {
      code: '<HogeFormControl><Input type="checkbox" /><Input type="checkbox" /></HogeFormControl>',
      errors: [{ message: invalidPureCheckboxInFormControl('Input') }],
    },
    {
      code: '<HogeFormControl><HogeCheckBox /><Input /></HogeFormControl>',
      errors: [{ message: invalidMultiInputsInFormControl() }],
    },
    {
      code: '<HogeFormControl><HogeCheckBox /><HogeCheckBox /></HogeFormControl>',
      errors: [{ message: invalidCheckboxInFormControl('HogeCheckBox') }],
    },
    {
      code: '<HogeFormControl><input type="radio" /></HogeFormControl>',
      errors: [{ message: invalidPureRadioInFormControl('input') }],
    },
    {
      code: '<HogeFormControl><RadioButton /></HogeFormControl>',
      errors: [{ message: invalidRadioInFormControl('RadioButton') }],
    },
    {
      code: '<HogeFormControl><HogeRadioButtonPanel /></HogeFormControl>',
      errors: [{ message: invalidRadioInFormControl('HogeRadioButtonPanel') }],
    },
    {
      code: '<HogeFieldset><HogeCheckBox /><HogeInput /></HogeFieldset>',
      errors: [{ message: noLabeledInputInFieldset('HogeInput') }],
    },
    {
      code: '<HogeFieldset><HogeCheckBox /><HogeSelect /></HogeFieldset>',
      errors: [{ message: noLabeledInputInFieldsetWithSelect('HogeSelect') }],
    },
    {
      code: '<FugaSection><HogeInput /></FugaSection>',
      errors: [{ message: useFormControlInsteadOfSection('HogeInput', 'FugaSection') }],
    },
    {
      code: '<FugaSection><HogeRadioButton /></FugaSection>',
      errors: [{ message: useFormControlInsteadOfSectionInRadio('HogeRadioButton', 'FugaSection') }],
    },
    { code: `const AnyHoge = () => <input />`, errors: [{ message: noLabeledInput('input') }] },
    {
      code: '<HogeFieldset role="group"><HogeFormControl /><HogeRadioButton /></HogeFieldset>',
      errors: [{ message: invalidFieldsetHasRoleGroup('HogeFieldset', 'smarthr-ui/Fieldset') }],
    },
    {
      code: '<fieldset role="group"><HogeFormControl /><HogeRadioButton /></fieldset>',
      errors: [{ message: invalidFieldsetHasRoleGroup('fieldset', 'fieldset') }],
    },
    { code: '<FormControl><HogeFieldset /></FormControl>', errors: [{ message: invalidChildreninFormControl('HogeFieldset') }] },
    {
      code: '<FormControl><HogeFormControl /></FormControl>',
      errors: [{ message: invalidChildreninFormControl('HogeFormControl') }],
    },
    {
      code: '<HogeFormControl role="group"><HogeInput /></HogeFormControl>',
      errors: [{ message: requireMultiInputInFormControlWithRoleGroup() }],
    },
    {
      code: '<HogeFormControl><HogeRadioButtons /></HogeFormControl>',
      errors: [{ message: invalidRadioInFormControl('HogeRadioButtons') }],
    },
    { code: '<HogeFormControl><HogeCheckBoxes /></HogeFormControl>', errors: [{ message: invalidMultiInputsInFormControl() }] },
    {
      code: '<HogeFormControl>{ dateInput ? <DateInput /> : <Input /> }<CheckBox /></HogeFormControl>',
      errors: [{ message: invalidMultiInputsInFormControl() }],
    },
