...
const errorMessage = (
  type,
  name,
) => `${name}には子要素が一つしか無いため、${type}でマークアップする意味がありません。${DETAIL_LINK_MESSAGE}
...
ruleTester.run('best-practice-for-layouts', rule, {
  ...
  invalid: [
    ...
    {
      code: `<HogeStack gap={0}>{a}{b}</HogeStack>`,
      errors: [
        {
          message: `HogeStack に "gap={0}" が指定されており、smarthr-ui/Stack の利用方法として誤っている可能性があります。以下の修正方法を検討してください。${DETAIL_LINK_MESSAGE}
 ...
 - 方法3: 別要素でマークアップし直すか、HogeStackを削除してください
   ...
   - as, forwardedAsなどでSectioningContent系要素に変更している場合、対応するsmarthr-ui/Section, Aside, Nav, Article のいずれかに差し替えてください`,
    ...
    {
      code: `<AnyHeading><AnyStack><Hoge /><Fuga /></AnyStack></AnyHeading>`,
      errors: [
        {
    ...
    {
      code: `<Heading><AnyIcon text="hoge" /></Heading>`,
      errors: [
        {
    ...
    {
      code: `<AnyHeading><Text prefixIcon={<SomeIcon />}>hoge</Text></AnyHeading>`,
      errors: [
        {
          message: `HeadingにIconを設定する場合 <Heading icon={<XxxIcon />}></Heading> のようにicon属性を利用してください${DETAIL_LINK_MESSAGE}`,
        },
    ...
    {
      code: `<FormControl label={<AnyStack><Hoge /><Fuga /></AnyStack>} />`,
      errors: [
        {
    ...
    {
      code: `<AnyFormControl label={{ text: <AnyIcon text="hoge" /> }} />`,
      errors: [
        {
    ...
    {
      code: `<FormControl label={{ text: <Text prefixIcon={<SomeIcon /> }>hoge</Text>}} />`,
      errors: [
        {
          message: `FormControlのlabel属性にアイコンを設定する場合 <FormControl label={{ text: 'テキスト', icon: <XxxIcon /> }} /> のようにlabel.icon属性を利用してください${DETAIL_LINK_MESSAGE}`,
        },
    ...
    {
      code: `<AnyFieldset legend={{ text: <AnyStack><Hoge /><Fuga /></AnyStack> }} />`,
      errors: [
        {
    ...
    {
      code: `<Fieldset legend={<AnyIcon text="hoge" />} />`,
      errors: [
        {
    ...
    {
      code: `<AnyFieldset legend={{ text: <Text prefixIcon={<SomeIcon />}>hoge</Text>}} />`,
      errors: [
        {
          message: `Fieldsetのlegend属性にアイコンを設定する場合 <Fieldset legend={{ text: 'テキスト', icon: <XxxIcon /> }} /> のようにlegend.icon属性を利用してください${DETAIL_LINK_MESSAGE}`,
        },
    ...
    {
      code: `<RadioButtonPanel><AnyStack><A /><B /></AnyStack></RadioButtonPanel>`,
      errors: [
        {
          message: `RadioButton, RadioButtonPanelの子孫にStackを置く場合、as属性、もしくはforwardedAs属性に \`span\` を指定してください${DETAIL_LINK_MESSAGE}`,
        },
      ],
    },
    {
      code: `<AnyCheckbox><Sidebar><A /><B /></Sidebar></AnyCheckbox>`,
      errors: [
        {
          message: `Checkboxの子孫にSidebarを置く場合、as属性、もしくはforwardedAs属性に \`span\` を指定してください${DETAIL_LINK_MESSAGE}`,
        },
      ],
    },
  ],
